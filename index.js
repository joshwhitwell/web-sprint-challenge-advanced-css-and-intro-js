...
function get20s(array, min, max){
  ...
  for (let i = 0; i < array.length; i++) {
    ...
    let yearDied = +array[i].years.slice(7, 11);
    if (yearBorn >= min && yearDied <= max) {
...
function addArtist(array, obj){
  array.push(obj);
  array[array.length-1].id = array.length - 1;
  return array;
  }
 ...
 "bio": "I am a Programs Manager at the Mind & Life Institute, where my primary responsibility is to oversee the planning and execution for our organization's yearly Summer Research Institute. I also support the design and implementation of our various scholarship funding mechanisms as well application review processes. I hold a Master’s degree in Buddhist Studies from the University of California, Berkeley, where my research focused on the early meditation traditions of Chinese Chan (Zen) and Tiantai Buddhism.",
 "wikipedia": "",
 "paintings": .5} 

 addArtist(artists, joshWhitwell);
 console.log(artists);
